...
for i in range(cloud.points.shape[0]):
    R = Rotation.from_euler('xyz', [cloud.points.roll[i],cloud.points.pitch[i],cloud.points.yaw[i]+50/180*math.pi], degrees=False).as_matrix()
    t = np.matrix([cloud.points.x[i], cloud.points.y[i], cloud.points.z[i]]).T
    correct = Rotation.from_euler('xyz', [0,0,-50/180*math.pi], degrees=False).as_matrix()
    t = correct*t
    T = np.eye(4)
    T[:3,:3] = R
    T[:3,3] = t.T
