...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void loopClosureThread()
    {
        ...
        ros::Rate rate(loopClosureFrequency);//回环检测主进程
        while (ros::ok())
        {
            rate.sleep();
            performLoopClosure();//回环检测
            visualizeLoopClosure();//可视化
        }
    }

    void loopInfoHandler(const std_msgs::Float64MultiArray::ConstPtr &loopMsg)//发消息的别管
    {
    ...
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;//复制一下点云一会用
        mtx.unlock();
    ...
    bool detectLoopClosureDistance(int *latestID, int *closestID)//回环判断俩是不是足够远
    {
    ...
    bool detectLoopClosureExternal(int *latestID, int *closestID)//距离之外的判断
    {
    ...
    void updateInitialGuess()
    {
        ...
        if (cloudInfo.vinsOdomAvailable == true && cloudInfo.vinsOdomResetId == vinsOdomResetId)//没有重启
        {
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void transformUpdate()
    {
        ...
        transformTobeMapped[0] = constraintTransformation(transformTobeMapped[0], rotation_tollerance);//限幅
    ...
    void publishOdometry()//update our EKF
    {
